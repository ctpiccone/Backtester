...
class TestInitialize(unittest.TestCase):
    def setUp(self) -> None:
        self.dh = AlpacaData.BarsAlpacaDataHandler(tickers, interval)
...
class TestGetLatest(unittest.TestCase):
    ...
    def test_get_multiple_latest(self):
        ...
        self.dh.update(self.dh.update(self.dh.update(self.dh.update(self.dh.get_latest_datetime("AAPL")))))
        self.assertTrue(self.dh.data["AAPL"].iloc[0:2].equals(self.dh.get_latest("AAPL", 2)))

    def test_ask_for_more(self):
        #Update AAPL 3 times, only ask for recent 2
        cur_datetime = START_DATE
        self.dh.update(self.dh.update(self.dh.update(self.dh.get_latest_datetime("AAPL"))))
        self.assertTrue(self.dh.data["AAPL"].iloc[[0]].equals(self.dh.get_latest("AAPL", 4)))
...
class TestGetLatestDate(unittest.TestCase):
    ...
    def test_updated_date(self):
        self.dh.update(self.dh.data["AAPL"].index[1])
        self.assertTrue(type(self.dh.get_latest_datetime("AAPL")) == pd._libs.tslibs.timestamps.Timestamp)
        self.assertTrue(self.dh.get_latest_datetime("AAPL").tzinfo == datetime.timezone.utc)
    ...
    def test_get_multiple_latest_dates(self):
        # Update AAPL 3 times, only ask for recent 2
        self.dh.update(self.dh.update(self.dh.update(self.dh.update(self.dh.update(self.dh.get_latest_datetime("AAPL"))))))
        self.assertEquals(list(self.dh.data["AAPL"].index[0:3]), list(self.dh.get_latest_datetime("AAPL", 3)))

    def test_ask_for_more_dates(self):
        # Update AAPL 3 times, only ask for recent 2
        self.dh.update(self.dh.update(self.dh.update(self.dh.update(self.dh.update(self.dh.get_latest_datetime("AAPL"))))))
        self.assertEqual(list(self.dh.data["AAPL"].index[0:3]), list(self.dh.get_latest_datetime("AAPL", 5)))

class TestUpdate(unittest.TestCase):

    def setUp(self) -> None:
        AlpacaData.BarsAlpacaDataHandler.OFFSET = datetime.timedelta(hours=2)
        self.dh = AlpacaData.BarsAlpacaDataHandler(tickers, interval)
        self.dh.set_dates(start_date=START_DATE, end_date=END_DATE)
        self.dh.initialize()
        self.dh.update(START_DATE)
    ...
    def test_data(self):
        ...
        while True:
            self.dh.update(cur_datetime)
            cur_datetime = cur_datetime + datetime.timedelta(milliseconds=5000000)
